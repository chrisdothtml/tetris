...
import { createBrain, DIRECTION_MAP, flattenGrid } from '../ai/brain.js'
...
function trainBrain (grid, outputs) {
  const output = DIRECTION_MAP.map(direction => {
    return outputs.has(direction) ? 1 : 0
  })

  BRAIN.activate(flattenGrid(grid))
  BRAIN.propagate(0.3, output)
}
  ...
  p5.draw = () => {
    if (GAME.gameOver) {
      ...
      if (TRAINING_INPUTS.size) {
        trainBrain(GAME.grid, TRAINING_INPUTS)
