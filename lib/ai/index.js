...
async function getNextMoves (game, network) {
  ...
  return getBestSim(sims).history.slice(0, 10)
}
...
;(async () => {
  ...
  await asyncWhile(() => true, async () => {
    const game = new Game({ seed: 'tetris' })
    ...
    await asyncWhile(() => !game.gameOver, async () => {
      ...
      for (const nextMove of nextMoves) {
        const input = nextMove.grid.reduce((result, row) => result.concat(row), [])
        const output = nextMove.move
        const directions = getDirectionsFromOutput(output)

        brain.activate(input)
        brain.propagate(learningRate, output)
