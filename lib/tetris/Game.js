...
import seedrandom from '../deps/seedrandom.js'

export default class Game {
  constructor (options = {}) {
    const { clone, seed } = options

    if (clone instanceof Game) {
      Object.assign(this, clone)

      // clone rng
      this.rng = seedrandom(null, { state: this.rng.state() })

      // clone items
      ;['item', 'nextItem'].forEach(itemKey => {
        if (this[itemKey]) {
          this[itemKey] = this[itemKey].clone()
        }
      })
    } else {
      this._grid = []
